...
export async function editUser(request: FastifyRequest, reply: FastifyReply): Promise<void> {
	...
	console.log("old = ", username, " new = ", newUsername, " pass = ", password, " new pass = ", newPassword, " av = ", avatar);
	try {
		...
		if (user) {
			//if (user.status == 1) {
				...
				const result = stmt.run(username, password, newUsername, newPassword);
				//if (result.changes > 0) {
				//	console.log("User edited: " + newUsername);
				return reply.send({ message: "yay this worked :)" });
					//return reply.send({ message: `User successfully edited: ${username} to ${newUsername}, ${newPassword}` });
				//}
				//else
